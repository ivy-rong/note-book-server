...
//根据笔记ID 获取当前所有内容
router.get('/contents/:noteId', async (req: Request, res: BaseResponse) => {
  const { noteId } = req.query
  try {
    const notes = await ContentsService.getContents(Number(noteId))
    res.status(200).json({
      message: '获取单个笔记数据成功',
      data: {
        contents: notes
...
router.get('/note/:noteId', async (req: Request, res: BaseResponse<Note>) => {
  ...
  try {
    // const note = await ContentsService.getNote(id, Number(noteId))
    res.status(200).json({
      message: '获取笔记成功'
      // data: note
    })
